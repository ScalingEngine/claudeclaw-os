...
must_haves:
  truths:
    - "A permanent BotFather bot is registered with handle @ezra_claudeclaw_bot (or the documented fallback if taken)"
  ...
  key_links:
    - from: "@ezra_claudeclaw_bot (BotFather)"
      ...
      pattern: "getMe.username matches ezra.*_claudeclaw(_io)?_bot"
...
BotFather registration flow (Telegram UI, no CLI):
  ...
  4. Username prompt: reply "ezra_claudeclaw_bot" (D-06 target; must end in _bot)
     Fallback if taken: "ezra_claudeclaw_io_bot" per CONTEXT.md specifics
...
Plan 04 and Plan 05 both run ClaudeClaw in Wave 3. Port 3141 can only be bound by one process at a time — each plan stops its process before exiting. Task 3 pre-flight checks port availability.
</interfaces>
</context>
...
<task type="checkpoint:human-action" gate="blocking">
  ...
  <how-to-verify>
    ...
    4. Username prompt → reply: ezra_claudeclaw_bot
       If taken, fall back to: ezra_claudeclaw_io_bot (or another ezra_*_claudeclaw_bot variant you pick). Record the final chosen handle.
...
<task type="auto">
  ...
  <action>
    ...
    2. Validate the token against Telegram's /getMe API:
       ...
       ```
       The response must have ok=true, result.is_bot=true, and result.username matching the regex ^ezra.*_claudeclaw(_io)?_bot$.
  ...
  <verify>
    <automated>TOKEN=$(grep '^TELEGRAM_BOT_TOKEN=' .env | head -1 | cut -d= -f2-) && [ -n "$TOKEN" ] && curl -sS "https://api.telegram.org/bot${TOKEN}/getMe" | node -e "const d=JSON.parse(require('fs').readFileSync(0,'utf8'));process.exit((d.ok&&d.result.is_bot&&/^ezra.*_claudeclaw(_io)?_bot$/.test(d.result.username))?0:1)" && grep -Pq '^ALLOWED_CHAT_ID=-?\d+$' .env && test -f .planning/phases/01-foundation/01-05-bot-identity.txt</automated>
  </verify>
  <acceptance_criteria>
    ...
    - Telegram API /getMe returns ok=true, is_bot=true, and username matches ^ezra.*_claudeclaw(_io)?_bot$ (permanent D-06/D-07 convention)
  ...
  <done>Token validates, handle matches D-06 convention, ALLOWED_CHAT_ID populated, identity artifact saved.</done>
</task>
...
<task type="auto">
  ...
  <action>
    Coordinate with Plan 04: if Plan 04 left its process running, stop it first. Expected starting state: port 3141 free and no ClaudeClaw process alive.

    1. Pre-flight:
       ```
       lsof -i :3141 -sTCP:LISTEN | grep -q node && { echo "Port 3141 busy — stop Plan 04 process first"; exit 1; } || true
...
<task type="auto">
  ...
  <verify>
    <automated>TOKEN=$(grep '^TELEGRAM_BOT_TOKEN=' .env | cut -d= -f2-) && ! lsof -i :3141 -sTCP:LISTEN 2>/dev/null | grep -q node && test -f .planning/phases/01-foundation/01-05-bot-runtime.log && ! grep -qF "$TOKEN" .planning/phases/01-foundation/01-05-bot-runtime.log && test -f /tmp/claudeclaw-phase1-bot.pid && false || true</automated>
  </verify>
  <acceptance_criteria>
...
| T-01-23 | Denial of service | Leftover process holds port 3141 | mitigate | Task 5 SIGTERM with 10s grace and SIGKILL fallback; acceptance verifies port released. |
...
- /getMe validates token and username matches D-06/D-07 convention
...
<success_criteria>
FOUN-06 satisfied: main Telegram bot responds to a test message from Noah's allowed chat ID. D-06 (permanent handle), D-07 (naming convention), D-08 (token env key), D-11 (single allowed chat) all implemented.
</success_criteria>

<output>
After completion, create `.planning/phases/01-foundation/01-05-SUMMARY.md` noting: final registered bot handle, whether fallback was used, approved timestamp of test reply, artifact paths (01-05-bot-identity.txt, 01-05-bot-runtime.log, optional 01-05-telegram-proof.png).
</output>
