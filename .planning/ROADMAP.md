...
- [x] Phase 3: Agent workflow routing policy — update Ezra, Vera, Poe, Cole, Hopper, and Archie personas with direct-answer vs skill/react-loop vs Archon-workflow guidance, including external-effect approval rules. Completed 2026-05-06. Requirements: ROUT-01, ROUT-02, ROUT-03, ROUT-04, ROUT-05.
...
- [ ] Phase 6: Scratchpad / compaction recovery — give every agent a per-turn working file to dump research findings into, plus persona-prompt rules so they re-Read it after `compact_boundary` fires. Surface the existing compaction warning *before* the empty-reply fallback fires (today it fires after, which is correct but useless). Per-agent prompt tuning so research-class agents (Vera, Archie) get explicit "append findings every 3 tool calls" framing while draft-class agents (Cole, Poe) get scratchpad-as-outline framing. Follow-up to the 2026-05-11 honesty patch (commit `c6756d7`); investigation captured in `.planning/debug/resolved/archie-compact-silent.md`. Requirements: SCRATCH-01, SCRATCH-02, SCRATCH-03, SCRATCH-04, SCRATCH-05.
- [ ] Phase 7: Timeout-aware reply with subagent + scratchpad salvage — when `AGENT_TIMEOUT_MS` fires at `bot.ts:658`, today's static "Timed out after 900s" message at `bot.ts:721` (and dashboard `:1810`) throws away (a) completed Task subagent reports already in the SDK message stream, and (b) the scratchpad file already on disk per Phase 6. Capture tool_result blocks on `user` events in `agent.ts` (currently ignored at the loop in `:289-413`); extend `AgentResult` with `subagentResults`; new `formatTimeoutReply(result, scratchpadPath)` helper concatenates subagent reports + scratchpad contents + partial assistant text; fall back to today's static message only when all sources are empty. Real-world evidence captured 2026-05-12 14:14 in `.planning/debug/archie-subagent-timeout-loss.md` (commit `47023d0`) — Archie's 2.7KB GHL workflows API research sat on disk while user saw the generic timeout message. Requirements: SALVAGE-01, SALVAGE-02, SALVAGE-03.
  - **Plans:** 3 plans
    - [ ] 07-01-PLAN.md — agent.ts tool_result capture + AgentResult extension (Wave 1, autonomous)
    - [ ] 07-02-PLAN.md — formatTimeoutReply helper + wire into both abort branches + readScratchpad (Wave 2, autonomous, depends on 07-01)
    - [ ] 07-03-PLAN.md — agent.test.ts + bot.test.ts coverage with Archie scratchpad fixture (Wave 3, autonomous, depends on 07-01 + 07-02)
